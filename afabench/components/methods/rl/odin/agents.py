...
@final
class ODINPolicyModule(nn.Module):
    def __init__(
        self,
        latent_size: int,
        n_actions: int,
...
@final
class ODINCommonModule(nn.Module):
    def __init__(
...
class ODINAgent(Agent):
    def __init__(
        ...
        self.common_module = ODINCommonModule(
        ...
        self.policy_head = ODINPolicyModule(
    ...
    def set_replay_buffer_device(self, device: torch.device) -> None:
        msg = "set_replay_buffer_device not yet supported for ODINAgent"
        raise ValueError(msg)
