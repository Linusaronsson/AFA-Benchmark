...
@final
class CubeNMARMARInitializer(AFAInitializer):
    """
    MAR control initializer for CubeNMARDataset.

    Like CubeNMARInitializer, rescue is censored with probability
    ``risky_rescue_missing_probability`` on risky-context instances.
    Unlike CubeNMARInitializer, context features are **pre-observed**
    at episode start, so the rescue-censoring decision depends only
    on already-observed features. This makes rescue missingness MAR
    by construction: J(r) = {context features} ⊆ S throughout.

    This is the key control experiment for the State-Dependent MAR
    Lemma: if the lemma holds, training under this initializer
    should produce policies with no stop bias and no routing
    distortion, matching cold-start performance.
    """
    ...
    def initialize(
        ...
        assert feature_shape is not None, (
            "feature_shape must be provided for CubeNMARMARInitializer"
        )
        ...
        # Pre-observe context features (the MAR difference).
        observed = torch.zeros_like(flat_features, dtype=torch.bool)
        observed[:, : self.n_contexts] = True
    ...
    def get_forbidden_selection_mask(
        ...
        if self._last_forbidden_selection_mask is None:
            msg = (
                "CubeNMARMARInitializer has no cached forbidden selection "
                "mask. Call initialize() before requesting forbidden "
                "selections."
            )
