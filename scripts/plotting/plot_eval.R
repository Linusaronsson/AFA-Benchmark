#!/usr/bin/env Rscript
#' Plot evaluation results from AFA method evaluation.
#'
#' Expected input CSV format:
#' - afa_method (str): name of the AFA method, e.g. "ODIN-MFRL" or "EDDI"
#' - classifier (str): which classifier is used to make the predictions. Values:
#'   "builtin" for method's built-in classifier, "external" for external classifier,
#'   or "none" if no classifier available.
#' - dataset (str): name of the dataset that the method is evaluated on,
#'   e.g. "cube" or "MNIST"
#' - selections_performed (int): how many AFA selections the method has performed
...
#'
#' Note: selections_performed and features_observed are usually the same, but not
#' always. For example, we can imagine a scenario where multiple features are
#' unmasked after each AFA action.
