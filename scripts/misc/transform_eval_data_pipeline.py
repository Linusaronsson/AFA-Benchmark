...
def main() -> None:
    ...
    parser.add_argument("--train_seed", type=str, help="Training seed. `null` if not applicable.")
    parser.add_argument("--train_hard_budget", type=str, help="Training hard budget. `null` if not applicable.")
    parser.add_argument(
        "--train_soft_budget_param", type=str, help="Training soft budget parameter. `null` if not applicable."
    )
    parser.add_argument(
        "--eval_soft_budget_param", type=str, help="Evaluation soft budget parameter. `null` if not applicable."
    )
    ...
    df = df.with_columns(
        ...
        train_hard_budget=pl.lit(parse_nullable(args.train_hard_budget), dtype=pl.Float64),
        train_soft_budget_param=pl.lit(parse_nullable(args.train_soft_budget_param), dtype=pl.Float64),
        eval_soft_budget_param=pl.lit(parse_nullable(args.eval_soft_budget_param), dtype=pl.Float64),
    )
